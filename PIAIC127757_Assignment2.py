...
def function6():
    ...
    arr = arr = np.zeros(10, dtype=int) #wrtie your code here
    arr[4]=10
    arr[7]=20
...
def function8():
    ...
    x = np.full_like(x, dtype=np.uint32, fill_value=6)
...
def function10():
    ...
    brr = np.array([1,2,3])
    subt = np.subtract(arr.transpose(), brr) # write your code here 
...
def function13():
    ...
    ans = np.take(arr, np.where((arr > 5) & (arr < 10))) #write your code here 
...
def function19():
    ...
    W = np.copy(X).T # Write np.copy(X).Tyour code here 
